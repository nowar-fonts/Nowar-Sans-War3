...
if __name__ == '__main__':
    ...
    # add phantom contours for Warcraft III to avoid trimming
    if "W3EU" in param["feature"]:
        for _, g in baseFont['glyf'].items():
            if 'contours' in g:
                w = g['advanceWidth']
                g['contours'] += [[{'x': 0, 'y': 0, 'on': True}, {'x': 0, 'y': 1, 'on': True}],
                                  [{'x': w, 'y': 0, 'on': True}, {'x': w, 'y': 1, 'on': True}]]
